...
// 当前页
	...
	var FB = function (data) {
		...
		// 触屏区域
		this.touchRectSize = new FB.Size(this.width / 2, this.height / 2);
	};
	/**
	 * 坐标类
	 * @param x
	...
	FB.prototype.initEvent = function () {
		...
		var _keyboard = function (e) {
			...
			switch (e.keyCode) {
				case previous:
					var sp = {
						x:100 * t.ratioWidth*xPercent,
						y:t.height - (150 * t.ratioHeight)*yPercent
					};
					keyTouch(sp);
					break;
				case next:
					var sp = {
						x: t.width *is_sing_page - (100 * t.ratioWidth)*xPercent,
						y:t.height - (150 * t.ratioHeight)*yPercent
					};
					keyTouch(sp);
					break;
     ...
     * 翻到指定页
     * */
    FB.prototype.currentRightPage = function (pageNum) {
        var podBox = this._bookData;
        var value =  pageNum;
        var isNext = value > this.currentPageIndex ? true : false;
        var currNum = value;
        var sp = {
            x: isNext ? (this.width * 1  )  - (100 * this.ratioWidth) : 100 * this.ratioWidth,
        ...
        if((isNext && this.currentPageIndex != currNum+1) || (!isNext && this.currentPageIndex != currNum+1)) {
            //保留上次页码
            this.lastPageIndex = this.currentPageIndex;
            //只翻偶数页
            this.currentPageIndex = currNum;
	...
	FB.prototype.aboutFlip = function (ops) {
		var xPercent = window.innerWidth / this.width,
			yPercent = window.innerHeight / this.height;
		var is_sing_page = this.isSinglePage()
		if(ops == 'next'){
			var sp = {
				x:this.width  *is_sing_page - (100*this.ratioHeight)* xPercent,
				y: this.height * xPercent -(150  *this.ratioHeight) * yPercent
			};
			this.keyTouch(sp);
		}
		...
		if(ops == 'previous'){
			var sp = {
				x:100 * this.ratioWidth* xPercent,
				y: this.height -(150  *this.ratioHeight)* xPercent
			};
			this.keyTouch(sp);
		}
	}
